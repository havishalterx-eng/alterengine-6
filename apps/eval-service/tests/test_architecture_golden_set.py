...
def _cases() -> list[Any]:
    ...
    return list(ARCHITECTURE_CASES)
...
def test_catalog_shape_and_ids() -> None:
    ...
    assert (golden_set.name, golden_set.domain, golden_set.version) == (
        "architecture",
        "architecture",
        1,
    )
    assert len(ARCHITECTURE_CASES) == 24
    assert len(_by_tag()) == 24

    ids = {case_id(golden_set, position) for position in range(1, 25)}
    assert len(ids) == 24
    other_ids = {
    ...
    assert groups == {"shape", "external", "gates", "pii", "residency"}
    for case in _cases():
...
def test_expected_gates_follow_the_gate_rules() -> None:
    for case in _cases():
        ...
        if constraints.get("human_approval_required") or constraints.get("customer_visible"):
            want |= {("human_approval", "before", key) for key in tools}
