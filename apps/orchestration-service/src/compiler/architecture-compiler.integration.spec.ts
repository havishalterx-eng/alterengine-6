...
function request(): ArchitectureCompileInput {
  return {
    ...
    architecture: { status: "ready", version: "1", topology: "single", boundaries: [{ kind: "verification", after_node_key: "work", reason: "required" }], success_criteria: ["Workflow is ready for review."], nodes: [{ source_node_key: "work", role: "direct", execution_kind: "llm", depends_on: [], capability_role: { source_node_key: "work", required_capabilities: ["text"], eligible_kinds: ["model"] }, success_criteria: ["Work is ready for review."] }], execution_waves: [{ order: 0, node_keys: ["work"], depends_on_wave_orders: [] }] },
  ...
  it("persists a valid pinned DAG under its tenant", async () => {
    ...
    const persisted = await store.withTenant(BARE_TENANT, (tx) => tx.query<{
      compiled_dag: {
        ...
        nodes: Array<{ key: string; success_criteria?: string[] }>;
      };
    ...
    expect(persisted.rows[0]!.compiled_dag.success_criteria).toEqual(["Workflow is ready for review."]);
  });
