terraform {
  ...
  backend "local" { path = "../../state/management.tfstate" }
}
...
provider "aws" {
  ...
  dynamic "assume_role" {
    ...
    content {
      ...
      session_name = "alter-management-terraform"
    }
  }
  default_tags { tags = { ManagedBy = "terraform", Project = "alterx", Account = "alter-management" } }
}
