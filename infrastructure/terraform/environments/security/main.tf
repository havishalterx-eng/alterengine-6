terraform {
  ...
  backend "local" { path = "../../state/security.tfstate" }
}
...
provider "aws" {
  ...
  dynamic "assume_role" {
    ...
    content {
      ...
      session_name = "alter-security-terraform"
    }
  }
  default_tags { tags = { ManagedBy = "terraform", Project = "alterx", Account = "alter-security" } }
}
module "environment" {
